					...
					afterChange: (changes) => {
						if (changes) {
							changes.forEach(([row, prop, oldValue, newValue]) => {
								if (prop === 'cm_mod_pos' && (newValue === 'MIC' || newValue === 'TRA')) {
									this.handsonTableService.cheminementsTable.setDataAtCell(row, this.handsonTableService.cheminementsTable.propToCol('cm_fildtec'), '1');
									return;
								}else if (prop === 'cm_mod_pos') {
									this.handsonTableService.cheminementsTable.setDataAtCell(row, this.handsonTableService.cheminementsTable.propToCol('cm_fildtec'), '0');
									return;
								}
							});
						}
...
export function cellNeedValue(instance, td, row, col, prop, value, cellProperties) {
	...
	if (prop === 'cm_mod_pos' || prop === 'cm_passage' || prop === 'cm_revet' || prop === 'cm_remblai' || prop === 'infra_type'  || prop === 'cm_r4_code'  || prop === 'cm_gest_do'  || prop === 'cm_fildtec') {
